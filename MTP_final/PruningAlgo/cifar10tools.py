...
class VGG(nn.Module):
    def __init__(self, features):
        ...
        if '28.weight' in self.features.state_dict().keys():
            inshape = self.features.state_dict()['28.weight'].shape[0]
        else:
            inshape = self.features.state_dict()['features.28.weight'].shape[0]
...
def get_model_filters_config(individual):
    filters_state = [
        ...
        'M',
        np.count_nonzero(individual[1152:1664]==1),
        np.count_nonzero(individual[1664:2176]==1),
        np.count_nonzero(individual[2176:2688]==1),
        'M',
        np.count_nonzero(individual[2688:3200]==1),
        np.count_nonzero(individual[3200:3712]==1),
        np.count_nonzero(individual[3712:4224]==1),
        'M'
    ]
...
def config_model_proper(individual, base_model):
    ...
    filters_state = {
        ...
        'features.17.weight': individual[1152:1664],
        'features.19.weight': individual[1664:2176],
        'features.21.weight': individual[2176:2688],
        'features.24.weight': individual[2688:3200],
        'features.26.weight': individual[3200:3712],
        'features.28.weight': individual[3712:4224]
    }
    ...
    for filter_idx, filter in enumerate(filters_state['features.14.weight']):
        if filter == 0:
            ...
            model_state_dict['features.17.weight'][:,filter_idx,:,:] = 0
    ## (8)
    for filter_idx, filter in enumerate(filters_state['features.17.weight']):
        if filter == 0:
            model_state_dict['features.17.weight'][filter_idx,:,:,:] = 0
            model_state_dict['features.17.bias'][filter_idx] = 0
    ...
    for filter_idx, filter in enumerate(filters_state['features.21.weight']):
        if filter == 0:
            ...
            model_state_dict['features.24.weight'][:,filter_idx,:,:] = 0
    ## (11)
    for filter_idx, filter in enumerate(filters_state['features.24.weight']):
        if filter == 0:
            model_state_dict['features.24.weight'][filter_idx,:,:,:] = 0
            model_state_dict['features.24.bias'][filter_idx] = 0

            model_state_dict['features.26.weight'][:,filter_idx,:,:] = 0
    ## (12)
    for filter_idx, filter in enumerate(filters_state['features.26.weight']):
        if filter == 0:
            model_state_dict['features.26.weight'][filter_idx,:,:,:] = 0
            model_state_dict['features.26.bias'][filter_idx] = 0

            model_state_dict['features.28.weight'][:,filter_idx,:,:] = 0
    
    ## (13) last conv layer is left unmodified to leave it stay compatible with fc layers
...
def make_model(individual, base_model):
    ...
    cfg_new = [
        ...
        'M',
        np.count_nonzero(filters_state['features.17.weight']==1),
        np.count_nonzero(filters_state['features.19.weight']==1),
        np.count_nonzero(filters_state['features.21.weight']==1),
        'M',
        np.count_nonzero(filters_state['features.24.weight']==1),
        np.count_nonzero(filters_state['features.26.weight']==1),
    ...
    for filter_idx, filter in enumerate(filters_state['features.14.weight']):
        if filter == 1:
            ...
            temp_state_dict['features.17.weight'][:,fil,:,:] = model_state_dict['features.17.weight'][filters_state['features.17.weight']==1,filter_idx,:,:]
            fil += 1
    ## (8)
    fil = 0
    for filter_idx, filter in enumerate(filters_state['features.17.weight']):
        if filter == 1:
            temp_state_dict['features.17.weight'][fil,:,:,:] = model_state_dict['features.17.weight'][filter_idx,filters_state['features.14.weight']==1,:,:]
            temp_state_dict['features.17.bias'][fil] = model_state_dict['features.17.bias'][filter_idx]
    ...
    for filter_idx, filter in enumerate(filters_state['features.19.weight']):
        if filter == 1:
            temp_state_dict['features.19.weight'][fil,:,:,:] = model_state_dict['features.19.weight'][filter_idx,filters_state['features.17.weight']==1,:,:]
    ...
    for filter_idx, filter in enumerate(filters_state['features.21.weight']):
        if filter == 1:
            ...
            temp_state_dict['features.24.weight'][:,fil,:,:] = model_state_dict['features.24.weight'][filters_state['features.24.weight']==1,filter_idx,:,:]
            fil += 1
    ## (11)
    fil = 0
    for filter_idx, filter in enumerate(filters_state['features.24.weight']):
        if filter == 1:
            temp_state_dict['features.24.weight'][fil,:,:,:] = model_state_dict['features.24.weight'][filter_idx,filters_state['features.21.weight']==1,:,:]
            temp_state_dict['features.24.bias'][fil] = model_state_dict['features.24.bias'][filter_idx]

            temp_state_dict['features.26.weight'][:,fil,:,:] = model_state_dict['features.26.weight'][filters_state['features.26.weight']==1,filter_idx,:,:]
            fil += 1
    ## (12)
    fil = 0
    for filter_idx, filter in enumerate(filters_state['features.26.weight']):
        if filter == 1:
            temp_state_dict['features.26.weight'][fil,:,:,:] = model_state_dict['features.26.weight'][filter_idx,filters_state['features.24.weight']==1,:,:]
            temp_state_dict['features.26.bias'][fil] = model_state_dict['features.26.bias'][filter_idx]
    ...
    for filter_idx, filter in enumerate(filters_state['features.28.weight']):
        if filter == 1:
            temp_state_dict['features.28.weight'][fil,:,:,:] = model_state_dict['features.28.weight'][filter_idx,filters_state['features.26.weight']==1,:,:]
